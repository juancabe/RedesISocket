...
char *preprocess_UDP_request(int s, struct sockaddr_in *clientaddr_in,
                             socklen_t *addrlen) {
  ...
  *addrlen = sizeof(struct sockaddr_in);
  ssize_t cc = recvfrom(s, buffer, TAM_BUFFER_IN_UDP - 1, 0,
...
void serverUDP(char *buffer, int s, struct sockaddr_in clientaddr_in,
               socklen_t addrlen) {
  ...
  if (sendto(s, response, strlen(response), 0,
             (struct sockaddr *)&clientaddr_in, addrlen) != strlen(response)) {
    errout("3rd ERROUT");
  }
